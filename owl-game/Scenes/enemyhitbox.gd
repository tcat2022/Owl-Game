...
func _ready() -> void:
	Health = MaxHealth #Sets enemy Health to max when game is loaded
...
func _on_area_entered(area: Area2D) -> void:
	...
	if (area.is_in_group("weaponhitbox")): #Checks if enemy's hitbox collided with weapon
		...
		EnemyHealthSystem(PlayerDamage, PlayerKnockback)
		#^^ Sends everything to Enemy health function
		print(Health)
		
		
func EnemyHealthSystem(PlayerDamage: int, 
PlayerKnockback: Vector2) -> void: #Enemy health Function
	Health = Health - PlayerDamage 
	healthChanged.emit()
	#Subtracts player damage from Enemy hp
	print(PlayerDamage)
	print(PlayerKnockback)
	
	
	if (Health > MaxHealth): #If enemy hp is greater then max hp, sets it to max hp
		Health = MaxHealth
